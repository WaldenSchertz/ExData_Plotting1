...
# Initialize 2x2 multi-plot
par(mfrow = c(2,2))

# Create line plot tracking Global Active Power over time (top left)
...
# Create line plot tracking Voltage over time (top right)
plot(data$Time, data$Voltage, type = "l", xaxt = "n",
     xlab = "datetime", ylab = "Voltage")
# Add custom x-axis ticks based on day
axis(1, at = as.numeric(c(min(data$Time), median(data$Time), max(data$Time))), 
     labels = c("Thu", "Fri", "Sat"))

# Plot three lines for three sub_metering variables (bottom left)
...
# Create line plot tracking Global Reactive Power over time (bottom right)
plot(data$Time, data$Global_reactive_power, type = "l", xaxt = "n",
